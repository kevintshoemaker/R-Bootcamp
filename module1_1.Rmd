...
To do this, click on the "folder" icon on the Rstudio toolbar (or File>>New File) and choose the script you downloaded earlier.
...
Scalars are the simplest objects in R. A scalar is just a single value. Remember the when we assigned the text string 'Alfred Pennyworth' to the `Batmans_butler` object? When we did that we created a scalar data object that contained a single text string. Scalars can be a single value of any data type.
