...
Objects are assigned a value using R's **assignment operator**, which looks like a left arrow (`<-`). Type the following statement into the console:
...
[^2]: RStudio has a useful auto-type feature, which can save you lots of time and headaches. After you've typed the first couple letters, RStudio will suggest "Batmans_butler" and you can just hit "Enter" to complete the object name!
...
1. Click on the top of the script (first line of comments). To **run** a line of code, press the "Ctrl" key (or the "Command" key) and hit "Enter". R will then run that line through the console. But of course, nothing happens because it's a comment and not meant to be interpreted by a machine[^3]. 
...
[^3]: RStudio has several helpful keyboard shortcuts to make running lines of code easier. [Check this website](https://support.rstudio.com/hc/en-us/articles/200711853-Keyboard-Shortcuts) for a comprehensive set of keyboard shortcuts

## R Objects

R has many different types of objects (sometimes refered to as data types). These are the building blocks of the language. They include scalars, vectors, matrices, lists, data frames, and functions.

### Workspace management

The workspace (environment) panel window in R can become cluttered as we create variables and assignments in R. As we work through the modules in this bootcamp it will be helpful to clear the workspace everyonce in a while to maintain our sanity. Earlier we assigned the value "Alfred Pennworth" to the variable `Batmans_butler`. Let's remove that from the workspace. There are several ways to do this. 

1. In the environment panel there is a little broom. If you click that it will clear the entire workspace.
...
Scalars are the simplest objects in R. A scalar is just a single value. Remember the when we assigned Alfred Pennyworth to the `Batmans_butler` object? When we did that we created a scalar. Scalars can be any single value.
...
Scalars do have a specific type. In the example above, `scalar1` is a character, `scalar2` and `scalar3` are both numeric. What is the data type for `scalar4`? It is a character. If you are uncertain about the type of any R object the R function [^4] `typeof()` will come in handy. Lets work through the code below. 
...
Vectors are combinations of scalars stored as a single object. Let's create some vectors.
...
There are many different ways to create a vector. You'll learn more about that later in the bootcamp. Look (or print) some of the vecotrs in the workspace. Each vector is comprised of several scalars. Each of these scalars is called an element of the vector. You can still check the types of the objects with the `typeof()` function (try it yourself).

What happened with `vector2` and `vector4`? We supplied a mix of numeric types and character, but the resulting vector is all characters? This is because vectors cannot contain multiple different types. R will convert (coerce) all types to a type that the data can be coerced to. Check `vector5`. The types of each element are logical, numeric, logical. In this case R has decided to convert the logical elements to numeric. `TRUE` maps to 1 and `FALSE` maps to 0.
...
One important concept to understand is that functions can be treated like regular objects in R. This is actually a very important concept in programming in general, and a feature of many programming languages. This feature, when objects can be treated as regular objects, is refered to [first-class functions](https://en.wikipedia.org/wiki/First-class_function). In practice this means that we can assign functions to values, use them as arguments in other functions, and return functions from functions. This sounds complicated, but work through the code below for some insight.
...
**Matrix** objects are just a bunch of vectors grouped together! To form a matrix, the vectors must be of the same length and the same **class** (class is another term for type, you'll learn more about classes soon). If different types are provided R will attempt to coerce the elements into a common type. A matrix has two **dimensions**: rows and columns.  
