...
myCluster <- parallel::makeCluster(spec=12,type="PSOCK")    # make cluster with 4 cpus of type "PSOCK"
myCluster
...
myCluster <- makeCluster(spec=12,type="PSOCK")
...
nruns <- 70000
tstfor <-data.frame( correlation = numeric(nruns))
system.time(
  for(i in 1:nruns){
  tstfor$correlation[i] <- cor(geneData[pair[[i]][1],],geneData[pair[[i]][2],])
}
)
  myfn <- function(x, geneData){
  thiscor <- cor(geneData[pair[[x]][1],],geneData[pair[[x]][2],])
  }
...
allruns <- seq(1:nruns)  
  system.time(
tstlapply <- mclapply(allruns, FUN = myfn,geneData=geneData, mc.cores = 11)
)
head(tstlapply)

tstlapplydata <- unlist(tstlapply)
  head(tstlapplydata)

  
## Runs on windoze based systems ####  
allruns <- seq(1:nruns) 
  system.time( # this includes the cluster startup in the time adds a couple of seconds
   { cl <- makeCluster(getOption("cl.cores", 11))
loadData <- parallel::clusterExport(cl=cl,c("geneData", "pair"))

tstparlapply <-  parLapply(cl = cl,allruns, fun = myfn,geneData = geneData)})
#or 
 system.time(
   tstparsapply <-  parSapply(cl = cl,allruns, FUN = myfn,geneData = geneData)
