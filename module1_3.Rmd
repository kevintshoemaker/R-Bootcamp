...
To get started with "ggplot()" we first give it a data set! 

We then usually define the "x" argument (defining coordinates on an x axis) and a "y" argument (defining coordinates on a y axis). In ggplot these are known as aesthetic mappings! That is because it is a way of conveying information in the dataset graphically!

Here is an example of a **scatterplot** in ggplot:  
...
```{r}
    ...
    # try representing tree height using color
ggplot(trees, aes(x=Girth,y=Volume)) +
...
We can do this using the `faceting` functions in ggplot. 
...
We could change the "mfrow" graphical parameter from c(1,1) to c(2,2): this means that we can fit four plots into a single window. 

An even more powerful solution is to use the function `layout()`. An example using `layout()` is below. 
...
Whenever you use `layout()` or `par()`, the graphics window will retain this layout for all subsequent plots. To start over (and return to the default graphical parameters), use `graphics.off()` to reset the plot. For example:
...
Here is a graphic illustrating the main plot symbols you can reference using 'pch':

![](points.png)

You might want to remember a couple favorites (for example, I like to use pch=20).
...
Then, let's name each element of our plot colors vector to match the species names:
...
Now, each color is directly associated with a species. We can now use a logical operation to generate a vector of colors for our plot:
...
Error bars can be added to scatter plots in a similar way. We'll plot error bars representing 2 standard deviations from the mean. The object that you called your barplot (`b`) is interpreted by R as the x values in the middle of each bar `b` (which are very hard to guess!). 
...
This workshop is NOT an intro to statistics! 
