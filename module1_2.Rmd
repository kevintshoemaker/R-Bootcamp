...
1. Click [here](module1_2.R) to download the script! Save the script to a convenient folder on your laptop.
...
- read.table      
     - reads any table, you can specify which format (very general)    
- read.csv        
...
1. Download the following data files and store them in a convenient file location (e.g., the R bootcamp folder where your scripts are already!)
   ...
   - [Tab delimited data file](data.dat)        
...
2. Try not to name an object similar/same as a function (e.g. don't name your data "data.frame")        
3. Can't start object names with a number      

## Basic data checking
As you've already seen, objects can be vectors, matrices, arrays, data frames, lists, and factors.

*Elements* of vectors and matrices can be numbers (numeric), characters, logicals (TRUE/FALSE), etc. 
...
Okay, we have a data frame. We know that!  What about the internals of the data frame? For this we can use the "str()" function, which tells us some more about what's IN our object:
...
First, make sure you download the [Turtle Data](turtle_data.txt) and save to your working directory. 
...
Sorting is another common data operation, which helps to visualize and organize data. Here's how you do it in R, continuing with the turtle data example:
...
BUT this just leads to problems in the end. My recommendation: just don't use it!!


2. When reading in data, R automatically codes character data (any column containing even one character element) as a **factor**. This often leads to problems, since factor data (while useful) are actually integers and can confuse novice (and non-novice) users. This is one of the most common problems that new R users face! 
...
1: Create a new data frame by reading in the file ["comm_data.txt"](comm_data.txt) in your module folder (and saving to your working directory). For the new data frame, include only the following columns: Hab_class, C_DWN, C_UPS, and rename the columns as: "Class","Downstream", "Upstream".
...
3: I am trying to create a data frame with only male turtles using the "which" argument.  Would this command work?
